...
public class Ui {
   ...
   //     for(Statement st:exeStack.getAll())
        ...
        PrgState state = new PrgState(exeStack,symbolTable,messages,s,fileTable,heap,IdGenerator.generateId());
        IRepository repo = new PrgRepository("log.txt");
        Controller ctrl = new Controller(repo);
        repo.addPrg(state);
        ctrl.allStep();
    ...
    public void run()
    {
        ...
        while(true)
        {
           try
           {
               ...
               {
                   Statement s11 = new CompoundStatement(
                           new AssignStatement("v", new ConstantExpression(10)),
                           new CompoundStatement(
                                   new NewStatement("a", new ConstantExpression(22)),
                                   new CompoundStatement(
